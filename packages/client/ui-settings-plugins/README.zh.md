...
Subagent 卡会同时暂存其权限开关、精确模型复选框与「未指定模型时的默认路由」。启用时必须至少选择一条适配器路由。默认路由下拉列出授权模型，外加一个保持委派调用继承父 Session 路由的「无默认」空选项；取消勾选作为默认模型的路由会一并清空暂存的默认路由。保存会在一次 mutation 中提交 `enabled`、`allowedModels` 与 `defaultRoute`，并以草稿开始时的 revision 设栅；Host revision 更新后，草稿会标记为失败，而不会恢复已撤销的路由。关闭时会保留已选路由供以后重新使用，并在同一次 mutation 中移除已存的默认路由。可用模型按提供方分组；当前目录中缺失的已存路由排在末尾，且仍可移除。适配器名称与模型描述仍属于实时目录元数据，不会存储；适配器变化、设置提交和重连后，卡片会刷新这些元数据。
