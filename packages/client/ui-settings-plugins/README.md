...
The Subagent card stages its permission switch, exact model checkboxes, and the no-selection default route together. Enabling requires at least one selected adapter route. The default route dropdown offers the authorized models plus a none option that keeps delegation calls inheriting the parent Session's route, and denying the model that is the default clears the staged default. Saving submits `enabled`, `allowedModels`, and `defaultRoute` in one mutation fenced by the revision where that draft began; a newer Host revision marks the draft failed instead of restoring a revoked route. Disabling retains the selected routes for later reuse and removes the stored default in the same mutation. Available models are grouped by provider, while saved routes absent from the current catalog appear last and remain removable. Adapter names and model descriptions remain live directory metadata and are not stored, and the card refreshes them after adapter changes, settings commits, and reconnects.
