...
The Host-owned `subagent-model-selection` settings section stores an explicit `enabled` switch, `allowedModels`, and optional `defaultRoute` and `arbiterRoute` values. Each configured route must be an exact `{ provider, model }` member of `allowedModels`; enabling requires at least one allowed route, and disabling may retain all route choices for later reuse. The Plugins settings card reads the live adapter directory through `session/modelCatalog`, lets the user stage the switch, authorization, and defaults, and saves them in one revision-fenced settings mutation. It stores no adapter-owned display names, descriptions, or reasoning-effort metadata. A stored or staged route absent from the current directory remains visible as unavailable and removable; a provider-local catalog failure does not block other providers or erase saved authorization or an unsaved selection. A connection reset discards the draft because namespace revisions are comparable only within one Host process.
